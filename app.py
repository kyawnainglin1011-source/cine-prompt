...
# 5. FIXED SYSTEM PROMPT (Syntax Error လုံးဝမတက်စေရန် ပြင်ဆင်ပြီး)
system_prompt = """
      ...
      "shots": [
        {
          ...
          "image_prompt": "English prompt for image generation: detailed {STYLE}, {CHAR_DESC}, cinematic lighting, 8k resolution, camera angle",
...
အရေးကြီးချက်: image_prompt နှင့် video_prompt များကို နိုင်ငံတကာ AI tools များတွင် တန်းသုံးနိုင်ရန် 'အင်္ဂလိပ်လို' သာ အမြဲတမ်း ထုတ်ပေးရမည်။ ပုံမထွက်သည့်ပြဿနာမရှိစေရန် image_prompt ထဲတွင် မြန်မာစာလုံးဝမပါရပါ။ အဖြေကို JSON သီးသန့်သာ ပြန်ပေးပါ။
"""
        ...
        for scene in data['scenes']:
            ...
            for idx, shot in enumerate(scene['shots']):
                ...
                with cols[idx % 2]:
                    ...
                    # 100% ပုံထွက်စေရန် ပြင်ဆင်ထားသော Image Loader
                    if show_storyboard:
                        # Special character များကို ရှင်းထုတ်ပြီး URL encode လုပ်သည်
                        clean_prompt = shot['image_prompt'].replace('"', '').replace("'", "").replace("\n", " ").strip()
                        encoded_prompt = urllib.parse.quote(clean_prompt)
                        # တရားဝင် ဖော်မတ်သစ် Endpoint ကို ပြောင်းလဲအသုံးပြုထားသည်
                        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1280&height=720&nologo=true&seed=100"
